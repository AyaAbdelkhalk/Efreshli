...
//            var addressIdStr = session.Metadata.TryGetValue("addressId", out var aid) ? aid : null;
//            var couponCode = session.Metadata.TryGetValue("couponCode", out var ccode) ? ccode : null;
...
//            {
//                // 1. Validate cart
//                var userCart = await _unitOfWork.CartRepository.GetAllWithIncludeAsync(
...
//                {
//                    // Fetch product item with concurrency token (optimistic lock)
...
//                    // Reduce stock now (transaction safe)
...
//                await _unitOfWork.SaveChangesAsync(); // Needed here to get OrderId
...
//                await _unitOfWork.SaveChangesAsync(); // Needed here to get PaymentId

//                order.PaymentId = payment.PaymentId;

//                // 7. Clear cart (bulk)

//                foreach (var it in userCart.Items)
//                {

//                    await _unitOfWork.CartItemRepository.RemoveAsync(it.CartItemId);
...
//                    if (user != null && !user.Coupons.Any(c => c.CouponId == coupon.CouponId))
...
namespace Efreshli.Application.Services.OrderServices
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<Response<Order>> CreateOrderFromStripeSessionAsync(Session session)
        {
            Console.WriteLine($"🔄 Starting order creation for session: {session.Id}");
            ...
            Console.WriteLine($"📋 Session metadata - UserId: {userId}, AddressId: {addressIdStr}, CouponCode: {couponCode}");

            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(addressIdStr) || !int.TryParse(addressIdStr, out var addressId))
            {
                Console.WriteLine("❌ Invalid session metadata");
                return ResponseHandler.BadRequest<Order>("Invalid session metadata");
            }
            ...
            if (existingPayment != null)
            {
                Console.WriteLine($"ℹ️ Order already exists for session {session.Id}");
                return ResponseHandler.Success(existingPayment.Order, "Order already created for this session.");
            }
            ...
            return await _unitOfWork.ExecuteInTransactionAsync(async () =>
            {
                Console.WriteLine($"🛒 Validating cart for user: {userId}");
                ...
                Console.WriteLine($"✅ Cart found with {userCart.Items.Count} items");

                // 2. Validate address
                var address = await _unitOfWork.AddressRepository.GetByIdAsync(addressId);
                if (address == null || address.ApplicationUserId != userId)
                {
                    Console.WriteLine($"❌ Invalid shipping address: {addressId}");
                    return ResponseHandler.BadRequest<Order>("Invalid shipping address.");
                }

                Console.WriteLine($"✅ Address validated: {addressId}");
                ...
                Console.WriteLine($"💰 Order totals - SubTotal: {subTotal}, Discount: {discountValue}, Shipping: {DEFAULT_SHIPPING_PRICE}, Total: {order.TotalPrice}");
                ...
                // 8. Clear cart
                Console.WriteLine($"🧹 Clearing cart for user: {userId}");
                foreach (var item in userCart.Items.ToList())
                {
                    await _unitOfWork.CartItemRepository.RemoveAsync(item.CartItemId, CancellationToken.None);
                }
                ...
                if (coupon != null && discountValue > 0)
                {
                    Console.WriteLine($"🎫 Consuming coupon: {couponCode}");
                    var user = await _unitOfWork.UserRepository.GetAll().Include(x => x.Coupons).FirstOrDefaultAsync(x => x.Id == userId);
                    user.Coupons.Add(coupon);
                    ...
                    await _unitOfWork.CouponRepository.UpdateAsync(coupon, CancellationToken.None);
                    await _unitOfWork.UserRepository.UpdateAsync(user);
                    Console.WriteLine($"✅ Coupon consumed successfully");
                }
